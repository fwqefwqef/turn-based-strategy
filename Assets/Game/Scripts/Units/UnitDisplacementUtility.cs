...
namespace Windy.Srpg.Game.Units
{
    ...
    public static class UnitDisplacementUtility
    {
        ...
        private static void ApplyDisplacementPlan(CustomUnit user, CustomUnit target, CustomCellGrid cellGrid, DisplacementPlan plan)
        {
        ...
        private static bool CanOccupyCandidate(Cell candidateCell, CustomUnit user, CustomUnit target, Cell simultaneouslyVacatedCell)
        {
            ...
            {
                return !HasExternalBlockingOccupant(candidateCell, user, target)
                    && candidateCell.CurrentUnits.OfType<Unit>().All(unit => unit == null || unit == user || unit == target || !unit.Obstructable);
            }

            return !HasExternalBlockingOccupant(candidateCell, user, target);
        }

        private static bool HasExternalBlockingOccupant(Cell cell, CustomUnit user, CustomUnit target)
        {
        ...
        private static void RefreshCellOccupancy(Cell cell)
        {
            ...
            cell.IsTaken = cell.CurrentUnits
                .OfType<Unit>()
                .Any(unit => unit != null && unit.Obstructable);
            CustomUnit.InvalidateAllCachedPaths();
        }

        private static void SnapUnitToCell(CustomUnit unit, Cell destinationCell, CustomCellGrid cellGrid)
        {
        ...
        private static Cell FindCell(CustomCellGrid cellGrid, Vector2 offsetCoord)
        {
