...
namespace Windy.Srpg.Game.UI
{
    ...
    {
        public static event Action<CustomUnit> SelectionTargetChanged;
        public static event Action<CustomUnit> InspectTargetChanged;
        ...
        private CustomUnit selectedUnit;
        private CustomUnit inspectedUnit;
        ...
        {
            if (CustomMoveAbility.SuppressInspectClicks)
            {
                return;
            }

            if (sender is not CustomUnit clickedUnit)
            {
                return;
            }
        ...
        {
            if (sender is not CustomUnit unit)
            {
                return;
            }
        ...
        {
            if (sender is not CustomUnit unit || selectedUnit != unit)
            {
                return;
            }
        ...
        {
            if (inspectedUnit == sender as CustomUnit)
            {
                UpdatePendingMoveAutoHideState();
                Refresh();
            }
        ...
        private void Refresh()
        {
            ...
            CustomUnit displayUnit = inspectedUnit;
            if (displayUnit == null)
            {
                Hide();
                return;
        ...
        private void SetSelectedUnit(CustomUnit unit, bool resetManualHideState)
        {
            bool changed = selectedUnit != unit;
            selectedUnit = unit;
        ...
        private bool HandleInspectKeyPressed()
        {
            ...
            if (TryGetUnitUnderPointer(out CustomUnit hoveredUnit))
            {
        ...
        private void OpenInspect(CustomUnit unit)
        {
            if (unit == null)
            {
                CloseInspect(clearRememberedTarget: true);
        ...
        private bool CanAdoptClickedUnit(CustomUnit clickedUnit)
        {
            CustomUnit gameplaySelectedFriendly = GetGameplaySelectedFriendlyUnit();
            if (gameplaySelectedFriendly == null || gameplaySelectedFriendly == clickedUnit)
            {
                return true;
            }

            return cellGrid?.CurrentCustomState is Windy.Srpg.Game.Grid.States.CustomUnitSelectedState
                && clickedUnit != null
                && !IsCurrentFriendlyUnit(clickedUnit);
        }

        private CustomUnit GetGameplaySelectedFriendlyUnit()
        {
            return subscribedUnits.FirstOrDefault(unit => IsGameplaySelectedUnit(unit) && IsCurrentFriendlyUnit(unit));
        }

        private bool IsGameplaySelectedUnit(CustomUnit unit)
        {
            return unit != null && unit.IsSelectedForTurn;
        }

        private bool IsCurrentFriendlyUnit(CustomUnit unit)
        {
            return unit != null && cellGrid != null && unit.PlayerNumber == cellGrid.CurrentCustomPlayerNumber;
        }
        ...
        private IEnumerable<UnitInspectEntryListUI.EntryData> BuildInventoryEntries(CustomUnit unit)
        {
        ...
        private IEnumerable<UnitInspectEntryListUI.EntryData> BuildSkillEntries(CustomUnit unit)
        {
        ...
        private IEnumerable<UnitInspectEntryListUI.EntryData> BuildPassiveEntries(CustomUnit unit)
        {
        ...
        private IEnumerable<UnitInspectEntryListUI.EntryData> BuildBuffEntries(CustomUnit unit)
        {
        ...
        private bool TryGetUnitUnderPointer(out CustomUnit unit)
        {
            unit = null;
            ...
            foreach (RaycastHit hit in hits)
            {
                ...
                CustomUnit hitUnit = hit.collider.GetComponentInParent<CustomUnit>();
                if (hitUnit != null)
                {
                    unit = hitUnit;
                    return true;
        ...
        private string BuildExperienceText(CustomUnit unit)
        {
            if (unit == null)
            {
