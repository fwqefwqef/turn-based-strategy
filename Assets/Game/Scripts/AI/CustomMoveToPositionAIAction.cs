using System.Collections;
using System.Collections.Generic;
using System.Diagnostics;
using System.Linq;
using System.Text;
using TbsFramework.Cells;
using Windy.Srpg.Game.AI;
using Windy.Srpg.Game.Units;
using Windy.Srpg.Game.Abilities;
using Windy.Srpg.Game.AI.Evaluators;
using Windy.Srpg.Game.Grid;
using Windy.Srpg.Game.Grid.States;
using Windy.Srpg.Game.Players;
using UnityEngine;

namespace Windy.Srpg.Game.AI.Actions
{
    public class CustomMoveToPositionAIAction : AIAction
    {
        public bool ShouldMoveAllTheWay = true;
        private Cell topDestination;

        private Dictionary<Cell, string> cellMetadata;
        private IEnumerable<(Cell cell, float value)> cellScores;
        private Dictionary<Cell, float> cellScoresDict;
        private Dictionary<string, Dictionary<string, float>> executionTime;

        private Gradient debugGradient;
        private Stopwatch stopWatch = new Stopwatch();

        private void Awake()
        {
            var colorKeys = new GradientColorKey[3];
            colorKeys[0] = new GradientColorKey(Color.red, 0.2f);
            colorKeys[1] = new GradientColorKey(Color.yellow, 0.5f);
            colorKeys[2] = new GradientColorKey(Color.green, 0.8f);

            debugGradient = new Gradient();
            debugGradient.SetKeys(colorKeys, new GradientAlphaKey[0]);
        }

        private static List<Cell> ResolveGridCells(CustomCellGrid cellGrid)
        {
            return cellGrid?.GetAllCells() ?? new List<Cell>();
        }

        public override void InitializeAction(CustomPlayer player, CustomUnit unit, CustomCellGrid cellGrid)
        {
            unit.GetComponent<CustomMoveAbility>()?.OnActionSelected(cellGrid);

            List<Cell> allCells = ResolveGridCells(cellGrid);
            cellMetadata = new Dictionary<Cell, string>();
            cellScoresDict = new Dictionary<Cell, float>();
            allCells.ForEach(c =>
            {
                cellMetadata[c] = "";
                cellScoresDict[c] = 0f;
            });

            executionTime = new Dictionary<string, Dictionary<string, float>>();
            executionTime.Add("precalculate", new Dictionary<string, float>());
            executionTime.Add("evaluate", new Dictionary<string, float>());
        }

        public override bool ShouldExecute(CustomPlayer player, CustomUnit unit, CustomCellGrid cellGrid)
        {
            if (unit == null || unit.GetComponent<CustomMoveAbility>() == null || unit.Cell == null)
            {
                return false;
            }

            var evaluators = GetComponents<CellEvaluator>();
            foreach (var evaluator in evaluators)
            {
                stopWatch.Start();
                evaluator.Precalculate(unit, player, cellGrid);
                stopWatch.Stop();

                executionTime["precalculate"].Add(evaluator.GetType().Name, stopWatch.ElapsedMilliseconds);
                executionTime["evaluate"].Add(evaluator.GetType().Name, 0);
                stopWatch.Reset();
            }

            List<Cell> allCells = ResolveGridCells(cellGrid);
            cellScores = allCells.Select(c => (cell: c, value: evaluators.Select(evaluator =>
            {
                stopWatch.Start();
                var score = evaluator.Evaluate(c, unit, player, cellGrid);
                stopWatch.Stop();
                executionTime["evaluate"][evaluator.GetType().Name] += stopWatch.ElapsedMilliseconds;
                stopWatch.Reset();

                var weightedScore = score * evaluator.Weight;
                if ((player as CustomAiPlayer)?.DebugMode ?? false)
                {
                    cellMetadata[c] += string.Format("{0} * {1} = {2} : {3}\n", evaluator.Weight.ToString("+0.00;-0.00"), score.ToString("+0.00;-0.00"), weightedScore.ToString("+0.00;-0.00"), evaluator.GetType());
                }

                cellScoresDict[c] += weightedScore;
                return weightedScore;
            }).DefaultIfEmpty(0f).Aggregate((result, next) => result + next))).OrderByDescending(x => x.value);

            HashSet<Cell> reachableDestinations = unit.GetAvailableDestinations(allCells);
            var reachableCellScores = cellScores
                .Where(o => reachableDestinations.Contains(o.cell))
                .ToList();

            if (reachableCellScores.Count == 0)
            {
                topDestination = unit.Cell;
                return false;
            }

            var (bestCell, maxValue) = reachableCellScores.First();
            var currentCellVal = cellScoresDict[unit.Cell];

            if (maxValue > currentCellVal)
            {
                topDestination = bestCell;
                return true;
            }

            if (TrySelectPursuitDestination(unit, player, cellGrid, out Cell pursuitDestination)
                && pursuitDestination != null
                && pursuitDestination != unit.Cell)
            {
                topDestination = pursuitDestination;
                return true;
            }

            topDestination = unit.Cell;
            return false;
        }

        public override void Precalculate(CustomPlayer player, CustomUnit unit, CustomCellGrid cellGrid)
        {
            if (unit == null || unit.Cell == null || topDestination == null)
            {
                topDestination = unit?.Cell;
                return;
            }

            List<Cell> allCells = ResolveGridCells(cellGrid);
            unit.CachePaths(allCells);
            var path = unit.FindPath(allCells, topDestination);
            var selectedPath = new List<Cell>();
            float cost = 0;

            for (int i = path.Count - 1; i >= 0; i--)
            {
                var cell = path[i];
                if (!unit.IsCellTraversable(cell) && !unit.IsCellMovableTo(cell))
                {
                    break;
                }

                cost += cell.MovementCost;
                if (cost <= unit.MovementPoints)
                {
                    selectedPath.Add(cell);
                }
                else
                {
                    break;
                }
            }

            if (selectedPath.Count != 0)
            {
                topDestination = ShouldMoveAllTheWay
                    ? selectedPath[selectedPath.Count - 1]
                    : selectedPath.OrderByDescending(c => cellScoresDict[c]).First();
            }
        }

        public override IEnumerator Execute(CustomPlayer player, CustomUnit unit, CustomCellGrid cellGrid)
        {
            var moveAbility = unit.GetComponent<CustomMoveAbility>();
            if (moveAbility == null || topDestination == null)
            {
                yield break;
            }

            moveAbility.Destination = topDestination;
            yield return moveAbility.AIExecute(cellGrid);
        }

        public override void CleanUp(CustomPlayer player, CustomUnit unit, CustomCellGrid cellGrid)
        {
            foreach (var cell in ResolveGridCells(cellGrid))
            {
                cell.UnMark();
            }

            topDestination = null;
            var aiTurnState = cellGrid.CurrentCustomState as CustomCellGridStateAiTurn;
            if (aiTurnState != null)
            {
                aiTurnState.CellDebugInfo = null;
            }
        }

        public override void ShowDebugInfo(CustomPlayer player, CustomUnit unit, CustomCellGrid cellGrid)
        {
            if (cellScores == null || !cellScores.Any())
            {
                return;
            }

            var cellDebugInfo = new Dictionary<Cell, AiDebugInfo>();

            var maxScore = cellScores.Max(x => x.value);
            var minScore = cellScores.Min(x => x.value);

            maxScore = maxScore.Equals(float.NaN) ? 0 : maxScore;
            minScore = minScore.Equals(float.NaN) ? 0 : minScore;

            var cellScoresEnumerator = cellScores.GetEnumerator();
            cellScoresEnumerator.MoveNext();
            var (topCell, _) = cellScoresEnumerator.Current;
            cellDebugInfo[topCell] = new AiDebugInfo(cellMetadata[topCell], Color.blue);

            while (cellScoresEnumerator.MoveNext())
            {
                var (cell, value) = cellScoresEnumerator.Current;
                var color = debugGradient.Evaluate((value - minScore) / (Mathf.Abs(maxScore - minScore) + float.Epsilon));
                cellMetadata[cell] += string.Format("Total: {0}", cellScoresDict[cell].ToString("0.00"));
                cellDebugInfo[cell] = new AiDebugInfo(cellMetadata[cell], color);
            }

            cellScoresEnumerator.Dispose();

            if (topDestination != null && cellDebugInfo.TryGetValue(topDestination, out var destinationDebugInfo))
            {
                destinationDebugInfo.Color = Color.magenta;
            }
            var aiTurnState = cellGrid.CurrentCustomState as CustomCellGridStateAiTurn;
            if (aiTurnState != null)
            {
                aiTurnState.CellDebugInfo = cellDebugInfo;
            }

            var evaluators = GetComponents<CellEvaluator>();
            var sb = new StringBuilder();
            var sum = 0f;

            sb.AppendFormat("{0} evaluators execution time summary:\n", GetType().Name);
            foreach (var evaluator in evaluators)
            {
                var precalculateTime = executionTime["precalculate"][evaluator.GetType().Name];
                var evaluateTime = executionTime["evaluate"][evaluator.GetType().Name];
                sum += precalculateTime + evaluateTime;

                sb.AppendFormat("total: {0}ms\tprecalculate: {1}ms\tevaluate: {2}ms\t:{3}\n",
                    (precalculateTime + evaluateTime).ToString().PadLeft(4),
                    precalculateTime.ToString().PadLeft(4),
                    evaluateTime.ToString().PadLeft(4),
                    evaluator.GetType().Name);
            }

            sb.AppendFormat("sum: {0}ms", sum.ToString().PadLeft(4));
            UnityEngine.Debug.Log(sb.ToString());
        }

        private bool TrySelectPursuitDestination(CustomUnit customUnit, CustomPlayer player, CustomCellGrid cellGrid, out Cell pursuitDestination)
        {
            pursuitDestination = customUnit?.Cell;
            if (customUnit == null || cellGrid == null || customUnit.Cell == null)
            {
                return false;
            }

            List<CustomUnit> enemies = cellGrid.GetEnemyUnits(player)
                .OfType<CustomUnit>()
                .Where(enemy => enemy != null && enemy.Cell != null && !enemy.ExcludedFromBattle)
                .ToList();
            if (enemies.Count == 0)
            {
                return false;
            }

            List<Cell> allCells = ResolveGridCells(cellGrid);
            customUnit.CachePaths(allCells);

            Cell bestAttackCell = null;
            float bestPathCost = float.PositiveInfinity;
            float bestAttackScore = float.NegativeInfinity;

            foreach (Cell candidateCell in allCells)
            {
                if (candidateCell == null)
                {
                    continue;
                }

                bool canStandHere = candidateCell == customUnit.Cell || customUnit.IsCellMovableTo(candidateCell);
                if (!canStandHere)
                {
                    continue;
                }

                bool attacksAnyEnemy = enemies.Any(enemy => customUnit.IsUnitAttackable(enemy, candidateCell));
                if (!attacksAnyEnemy)
                {
                    continue;
                }

                IList<Cell> path = candidateCell == customUnit.Cell
                    ? new List<Cell>()
                    : customUnit.FindPath(allCells, candidateCell);
                if (candidateCell != customUnit.Cell && (path == null || path.Count == 0))
                {
                    continue;
                }

                float pathCost = path.Sum(cell => cell.MovementCost);
                float attackScore = cellScoresDict != null && cellScoresDict.TryGetValue(candidateCell, out float score)
                    ? score
                    : 0f;

                bool isBetter = pathCost < bestPathCost
                    || (Mathf.Approximately(pathCost, bestPathCost) && attackScore > bestAttackScore);
                if (!isBetter)
                {
                    continue;
                }

                bestAttackCell = candidateCell;
                bestPathCost = pathCost;
                bestAttackScore = attackScore;
            }

            if (bestAttackCell == null)
            {
                return false;
            }

            pursuitDestination = bestAttackCell;
            return true;
        }
    }
}
