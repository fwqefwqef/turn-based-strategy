...
namespace Windy.Srpg.Game.Abilities
{
    public class MoveAbility : Ability
    {
        ...
        public BattleSquareCell Destination { get; set; }
        private IList<BattleSquareCell> currentPath;
        public HashSet<BattleSquareCell> availableDestinations;
        ...
        private readonly List<BattleSquareCell> pendingAttackPreviewCells = new List<BattleSquareCell>();
        private readonly List<Unit> pendingSkillTargets = new List<Unit>();
        private readonly List<BattleSquareCell> pendingSkillPreviewCells = new List<BattleSquareCell>();
        private readonly List<BattleSquareCell> pendingAreaSkillCenterCells = new List<BattleSquareCell>();
        private readonly List<BattleSquareCell> pendingAreaSkillRadiusCells = new List<BattleSquareCell>();
        private readonly List<Unit> pendingAreaSkillTargets = new List<Unit>();
        private readonly List<BattleSquareCell> pendingTradePreviewCells = new List<BattleSquareCell>();
        ...
        private BattleSquareCell selectedAreaSkillCenterCell;
        ...
        private readonly struct AreaLineProjection
        {
            ...
            public readonly BattleSquareCell Endpoint;
            public readonly List<BattleSquareCell> Cells;

            public AreaLineProjection(Vector2Int direction, BattleSquareCell endpoint, List<BattleSquareCell> cells)
            {
                Direction = direction;
                Endpoint = endpoint;
                Cells = cells;
        ...
        {
            public readonly BattleSquareCell Cell;
            public readonly bool Top;
            public readonly bool Right;
            public readonly bool Bottom;
            public readonly bool Left;

            public AreaBorderOutline(BattleSquareCell cell, bool top, bool right, bool bottom, bool left)
            {
                Cell = cell;
                Top = top;
                Right = right;
        ...
        private static BattleSquareCell FindCellByOffset(CellGrid cellGrid, Vector2Int offset)
        {
        ...
        private IEnumerator BeginPendingMovePreview(CellGrid cellGrid, BattleSquareCell destinationCell, IList<BattleSquareCell> path)
        {
        ...
        private IEnumerator CompletePendingMovePreview(CellGrid cellGrid, BattleSquareCell destinationCell, bool waitForPreviewCamera)
        {
            if (!UnitReference.HasPendingMove)
            {
                yield break;
        ...
        public void OnPendingMoveCellSelected(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (!awaitingSkillTargetSelection || awaitingAreaSkillConfirmation || !IsAreaSkill(selectedTargetingSkill))
            {
                return;
        ...
        public void OnPendingMoveCellDeselected(BattleSquareCell cell, CellGrid cellGrid)
        {
        }

        public void OnPendingMoveCellClicked(BattleSquareCell cell, CellGrid cellGrid)
        {
        ...
        {
            BattleSquareCell actingCell = GetActingCellForPendingActions(cellGrid);
        ...
        private bool IsPendingActingCell(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return false;
        ...
        private void HandlePendingAreaSkillCellInteraction(BattleSquareCell hoveredCell, CellGrid cellGrid)
        {
        ...
        private void ShowAreaSkillConfirmPopup(CellGrid cellGrid, BattleSquareCell centerCell, IReadOnlyList<Unit> affectedTargets)
        {
        ...
        private static void MarkAttackPreviewCell(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private static void ClearAttackPreviewCellMark(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private BattleSquareCell GetActingCellForPendingActions(CellGrid cellGrid)
        {
            if (UnitReference == null)
            {
                return null;
            ...
            BattleSquareCell frameworkActingCell = UnitReference.PreviewCell ?? UnitReference.Cell;
            ...
            BattleSquareCell runtimeActingCell = cellGrid.ResolveRuntimeActingCell(UnitReference);
        ...
        {
            return UnityEngine.Object.FindObjectOfType<CellGrid>();
        }
        ...
        private static void MarkSkillPreviewCell(BattleSquareCell cell, SkillHighlightMode highlightMode, CellGrid cellGrid, bool faint = false)
        {
            if (cell == null)
            {
                return;
        ...
        private static void ClearSkillPreviewCellMark(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private List<BattleSquareCell> GetLegalAreaSkillCenterCells(Skill skill, CellGrid cellGrid)
        {
            var results = new List<BattleSquareCell>();
            BattleSquareCell actingCell = GetActingCellForPendingActions(cellGrid);
        ...
        private BattleSquareCell GetClosestLegalAreaSkillCenterCell(Skill skill, BattleSquareCell hoveredCell, CellGrid cellGrid)
        {
            if (hoveredCell == null)
            {
                return null;
        ...
        private Vector2Int ResolveLineAreaDirection(BattleSquareCell hoveredCell, CellGrid cellGrid)
        {
            BattleSquareCell actingCell = GetActingCellForPendingActions(cellGrid);
        ...
        private List<AreaBorderOutline> BuildAreaBorderOutlines(IEnumerable<BattleSquareCell> cells, CellGrid cellGrid)
        {
            var legalCenters = cells?
                .Where(cell => cell != null)
                .Distinct()
                .ToList() ?? new List<BattleSquareCell>();
            ...
            var legalSet = new HashSet<BattleSquareCell>(legalCenters);
        ...
        private Dictionary<Vector2Int, BattleSquareCell> BuildCellLookup(CellGrid cellGrid)
        {
        ...
        private AreaLineProjection BuildLineAreaProjection(Skill skill, Vector2Int direction, CellGrid cellGrid)
        {
            ...
            {
                return new AreaLineProjection(direction, null, new List<BattleSquareCell>());
            }
            ...
            Dictionary<Vector2Int, BattleSquareCell> lookup = BuildCellLookup(cellGrid);
            var cells = new List<BattleSquareCell>();
            var added = new HashSet<BattleSquareCell>();
            BattleSquareCell endpoint = null;

            for (int distance = startDistance; distance <= endDistance; distance++)
            {
                Vector2Int centerCoord = sourceCoord + direction * distance;
                if (lookup.TryGetValue(centerCoord, out BattleSquareCell centerCell))
                {
                    endpoint = centerCell;
                }
                ...
                for (int offset = -halfWidth; offset <= halfWidth; offset++)
                {
                    ...
                    if (!lookup.TryGetValue(targetCoord, out BattleSquareCell cell))
                    {
                        continue;
                    }
        ...
        private List<Unit> GetAreaSkillTargets(Skill skill, BattleSquareCell centerCell, CellGrid cellGrid)
        {
            ...
            HashSet<BattleSquareCell> affectedCells = GetAreaSkillAffectedCells(skill, centerCell, cellGrid);
            ...
            foreach (var unit in ResolveGridUnits(cellGrid))
            {
                ...
                BattleSquareCell unitCell = unit.HasPendingMove ? unit.PreviewCell : unit.Cell;
        ...
        private HashSet<BattleSquareCell> GetAreaSkillAffectedCells(Skill skill, BattleSquareCell centerCell, CellGrid cellGrid)
        {
            var results = new HashSet<BattleSquareCell>();
        ...
        private SkillContext BuildAreaSkillContext(Skill skill, BattleSquareCell centerCell, Unit primaryTarget, CellGrid cellGrid, IReadOnlyList<Unit> areaTargets = null)
        {
        ...
        private bool ShouldSkipAreaSkillOverlayCell(Skill skill, BattleSquareCell cell, CellGrid cellGrid)
        {
            BattleSquareCell actingCell = GetActingCellForPendingActions(cellGrid);
        ...
        private void UpdateAreaSkillProjection(BattleSquareCell hoveredCell, CellGrid cellGrid)
        {
            ...
            BattleSquareCell snappedCenter = IsLineAreaSkill(selectedTargetingSkill)
                ? hoveredCell
            ...
            HashSet<BattleSquareCell> affectedCells = GetAreaSkillAffectedCells(selectedTargetingSkill, snappedCenter, cellGrid);
        ...
        private static void MarkTradePreviewCell(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private static void ClearTradePreviewCellMark(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private IEnumerator ExecuteAreaSkillThenConfirmPendingMove(Skill skill, BattleSquareCell centerCell, IReadOnlyList<Unit> affectedTargets, CellGrid cellGrid)
        {
        ...
        private bool TryResolveSkillRange(Skill skill, WeaponData backingWeapon, BattleSquareCell sourceCell, out int minRange, out int maxRange)
        {
            minRange = 0;
            maxRange = 0;
        ...
        private Item GetPreferredWeaponForSkill(Skill skill, Unit target, BattleSquareCell sourceCell, Item preferredWeaponEntry = null)
        {
        ...
        private static void ApplySkillPreviewHighlight(BattleSquareCell cell, SkillHighlightMode highlightMode, bool faint = false)
        {
            CellHighlightKind kind = highlightMode switch
            {
                SkillHighlightMode.Enemy => CellHighlightKind.Attack,
        ...
        private List<Item> GetSkillPreviewWeaponOptions(Skill skill, Unit target, CellGrid cellGrid)
        {
            ...
            return UnitReference.GetWeaponInventoryEntries()
                ...
                .Where(entry =>
                {
                    ...
                    BattleSquareCell targetCell = target?.HasPendingMove == true ? target.PreviewCell : target?.Cell;
                    if (targetCell == null)
                    {
                        return false;
                    }
        ...
        private bool CanSkillReachCellFromPreview(Skill skill, BattleSquareCell targetCell, CellGrid cellGrid)
        {
            BattleSquareCell actingCell = GetActingCellForPendingActions(cellGrid);
        ...
        {
            BattleSquareCell originCell = UnitReference?.Cell;
            if (originCell != null)
            {
        ...
        private static void MarkReachableCell(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private static void MarkPathCell(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private static void ClearCellMark(BattleSquareCell cell, CellGrid cellGrid)
        {
            if (cell == null)
            {
                return;
        ...
        private static BoardCell GetRuntimeBoardCell(BattleSquareCell cell)
        {
            return cell;
        }
