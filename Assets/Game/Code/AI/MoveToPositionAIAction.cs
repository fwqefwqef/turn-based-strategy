...
namespace Windy.Srpg.Game.AI.Actions
{
    public class MoveToPositionAIAction : AIAction
    {
        ...
        private void Awake()
        {
            debugGradient = new Gradient
            {
                colorKeys = new[]
                {
                    ...
                    new GradientColorKey(new Color(0.95f, 0.8f, 0.2f), 0.45f),
                    new GradientColorKey(new Color(0.25f, 0.8f, 0.35f), 1f),
                },
                alphaKeys = new[]
                {
        ...
        public override void InitializeAction(Player player, Unit unit, CellGrid cellGrid)
        {
            ...
            {
                ["precalculate"] = new Dictionary<string, float>(),
        ...
        {
            if (unit == null || unit.GetComponent<MoveAbility>() == null || unit.Cell == null)
            {
                return false;
            }
            ...
            cellScores = allCells.Select(c => (cell: c, value: evaluators.Select(evaluator =>
            {
                ...
                if ((player as AiPlayer)?.DebugMode ?? false)
                {
        ...
        public override void CleanUp(Player player, Unit unit, CellGrid cellGrid)
        {
            ...
            topDestination = null;
            var aiTurnState = cellGrid.CurrentState as CellGridStateAiTurn;
            if (aiTurnState != null)
            {
        ...
        private bool TrySelectPursuitDestination(Unit customUnit, Player player, CellGrid cellGrid, out BattleSquareCell pursuitDestination)
        {
            ...
            List<Unit> enemies = cellGrid.GetEnemyUnits(player)
                .OfType<Unit>()
