...
namespace Windy.Srpg.Game.Skills
{
    ...
    public sealed class UnitSkillList
    {
        ...
        public void SetOverchargeGrantedSkill(string skillId)
        {
            overchargeGrantedEntry = string.IsNullOrWhiteSpace(skillId) || !SkillRegistry.TryGet(skillId, out _)
                ? null
                : new Skill(skillId);
            MarkEntriesDirty();
        }
        ...
        {
            return entries.Contains(entry) || equipmentGrantedEntries.Values.Contains(entry) || entry == overchargeGrantedEntry;
        }
        ...
        private void RebuildCombinedEntriesIfNeeded()
        {
            ...
            if (overchargeGrantedEntry != null)
            {
                combinedEntries.Add(overchargeGrantedEntry);
            }
            combinedEntries.AddRange(entries);
            combinedEntries.AddRange(equipmentGrantedEntries.Values);
            if (overchargeGrantedEntry != null)
            {
                for (int i = combinedEntries.Count - 1; i > 0; i--)
                {
                    if (string.Equals(combinedEntries[i]?.SkillId, overchargeGrantedEntry.SkillId, StringComparison.OrdinalIgnoreCase))
                    {
                        combinedEntries.RemoveAt(i);
                    }
                }
            }
