...
namespace Windy.Srpg.Game.Editor
{
    public sealed class MapPainterWindow : EditorWindow
    {
        ...
        private const string FriendlyUnitPrefabPath = "Assets/Prefabs/FriendlyUnit.prefab";
        private const string EnemyUnitPrefabPath = "Assets/Prefabs/EnemyUnit.prefab";
        private const string DeploymentSlotPrefabPath = "Assets/Prefabs/DeploymentSlot.prefab";
        ...
        private void DrawPaletteSection()
        {
            ...
            switch (paintMode)
            {
                ...
                case PaintMode.MoveUnit:
                    EditorGUILayout.HelpBox(
                        unitPendingMove == null
                            ? "Click a unit, then click its destination tile. Floor tiles are not changed."
                            : $"Moving '{unitPendingMove.name}'. Click a valid destination tile, or cancel the move.",
                        MessageType.None);
                    if (unitPendingMove != null && GUILayout.Button("Cancel Unit Move"))
                    {
                        unitPendingMove = null;
                        SceneView.RepaintAll();
                    }
                ...
                case PaintMode.EraseUnit:
                    EditorGUILayout.HelpBox("Erase Unit removes only the unit occupying the targeted tile. Floor and map markers remain unchanged.", MessageType.None);
                    break;

                case PaintMode.Erase:
                    EditorGUILayout.HelpBox("Erase All removes any painted cell, deployment slot, reinforcement marker, and unit on the targeted coordinate.", MessageType.None);
                    break;
            }
        }
        ...
        private void OnSceneGui(SceneView sceneView)
        {
            ...
            if (paintMode == PaintMode.MoveUnit && current.type == EventType.KeyDown && current.keyCode == KeyCode.Escape)
            {
                unitPendingMove = null;
                current.Use();
                Repaint();
            ...
            bool acceptsDrag = paintMode != PaintMode.MoveUnit;
            if (current.type == EventType.MouseDown || (acceptsDrag && current.type == EventType.MouseDrag))
            {
        ...
        private bool TryPaintAtCoordinate(MapPainterSceneContext context, Vector2Int coordinate)
        {
            switch (paintMode)
            {
                ...
                case PaintMode.MoveUnit:
                    return SelectOrMoveUnitAtCoordinate(context, coordinate);

                case PaintMode.EraseUnit:
                    return EraseUnitAtCoordinate(context, coordinate);
        ...
        private bool SelectOrMoveUnitAtCoordinate(MapPainterSceneContext context, Vector2Int coordinate)
        {
            if (context == null)
            {
                unitPendingMove = null;
            ...
            if (unitPendingMove == null)
            {
                unitPendingMove = GetUnitAtCoordinate(context, coordinate);
                if (unitPendingMove == null)
                {
                    return false;
                }

                Selection.activeGameObject = unitPendingMove.gameObject;
                Repaint();
                SceneView.RepaintAll();
                return true;
            }

            Unit movingUnit = unitPendingMove;
            Cell destinationCell = GetCellAtCoordinate(context, coordinate);
            if (destinationCell == null || !destinationCell.IsTraversable)
            {
                Debug.LogWarning($"Map Painter: Cannot move '{movingUnit.name}' to {coordinate}; the floor tile is missing or impassable.");
                return false;
            }

            Vector2Int footprintSize = new Vector2Int(movingUnit.FootprintWidth, movingUnit.FootprintHeight);
            for (int y = 0; y < footprintSize.y; y++)
            {
                for (int x = 0; x < footprintSize.x; x++)
                {
                    ...
                    {
                        Debug.LogWarning($"Map Painter: Cannot move the {footprintSize.x}x{footprintSize.y} unit '{movingUnit.name}' to {coordinate}; tile {footprintCoordinate} is missing or impassable.");
                        return false;
                    }

                    Unit overlappingUnit = GetUnitAtCoordinate(context, footprintCoordinate);
                    if (overlappingUnit != null && overlappingUnit != movingUnit)
                    {
                        Debug.LogWarning($"Map Painter: Cannot move '{movingUnit.name}' to {coordinate}; '{overlappingUnit.name}' occupies tile {footprintCoordinate}.");
                        return false;
                    }
                }
            }

            Undo.RecordObject(movingUnit, "Move Painted Unit");
            Undo.RecordObject(movingUnit.transform, "Move Painted Unit");
            movingUnit.Cell = destinationCell;
            movingUnit.transform.position = destinationCell.transform.position;
            EditorUtility.SetDirty(movingUnit);
            EditorUtility.SetDirty(movingUnit.transform);
            EditorSceneManager.MarkSceneDirty(context.gameObject.scene);

            unitPendingMove = null;
            Selection.activeGameObject = movingUnit.gameObject;
            Repaint();
            SceneView.RepaintAll();
            return true;
        }

        private bool EraseUnitAtCoordinate(MapPainterSceneContext context, Vector2Int coordinate)
        {
